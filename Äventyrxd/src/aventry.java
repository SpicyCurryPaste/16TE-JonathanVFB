...
//		monsterlist.add();
	...
	public static void movement () {
		System.out.println("Type [N] to go North, [S] South, [W] West, [E] East or [R] to stay.");
		...
		char dir = command.charAt(0);
			...
			case 'r':
			case 'R': 
				System.out.println("See your status (s) or continue journeying? (j)");
				command = scnr.nextLine();
				char o = command.charAt(0);
				switch (o){
				case 's':
				case 'S': System.out.println("Endurance: " + Endurance + ", Health: " + Vitality + ", Gold: " + gold +"." );
		...
		public static class NPC {
			...
			public NPC (String npName, String npDialogue) {
				//NPC info
				
				this.npName = npName;
				this.Dialogue = npDialogue;
				
				
			}
		}
		
		public static void npSPawn () {
			
			int dd = 0 + (int)(Math.random() * ((9 - 0) + 1)); 
			
			if (dd == 1) {
				
				if (Math.random() < 0.5) {
					NPC n = new NPC ("Yaba", "Hello, traveler! If you're hurt i believe my ointments could help! (Pay 10 gold to restore health?)" );
					System.out.println(n.npName+ ": " + n.Dialogue);
					command = scnr.nextLine();
					char bartr = command.charAt(0);
...
/*			int rn = 0 + (int)(Math.random() * ((monsterlist.size() - 0) + 1));
			...
			Monster o = new Monster ("Snake", 15, 3, 0);*/
				...
				if (Math.random() <0.5) {
					Monster m = new Monster("Snake",15,3,0);
		...
		// Use Rock, paper, scissors method.
			...
			String[] weapons = {"Hands","Sword","Shield"} ;

			while ( 0 < health && 0 < m.hp){
				
				System.out.println("Hands[0], Sword[1], Shield[2]");
				...
				else if ((player == enemy + 1)  || (enemy - player) == 2 ) {
					//win
					delay(2000);
					m.hp -= Damage;
					System.out.println("You: "+ weapons[player] + "  Monster: " + used );
					delay(1000);
					System.out.println("You used " + weapons[player] + " and dealt " + Damage + " to " + m.mName + "! Monster HP left: " + m.hp );
				...
				if (m.hp < 0) {
					...
					System.out.println("You received 10 gold and gained " + experienceReceived + " Exp!");
					leveling();
		...
		public static void leveling () {
			...
			if (experience == level + 10*2 ) {
				...
				delay(2000);
				System.out.println("Which attribute would you like to increases? (increase one of the attributes by 2)" + "Health: " + health + ", Stamina:" + stamina + ", Strength: " + strength + ", Intelligence: " + intelligence);
				delay(2000);
				System.out.println("Increase one attribute by entering one of the following: str, stmn, intl or hlth");
				command = scnr.nextLine();
		...
		public static void pDeath () {
			if (Vitality == 0) {
				System.out.println("Level upon death: " + level );
				delay(5000);
				System.exit(3);
			}
		}
